 ...
 * - WSL:     `powershell.exe -EncodedCommand` (Windows WinRT Toast)
...
/** Minimal structural type compatible with opencode's BunShell `$` */
    ...
    switch (platform) {
      case "linux":
        try {
          ...
          await notifyLinuxDBus(shell, title, body)
        }
        break
...
function buildToastScript(title: string, body: string): string {
  ...
  return [
    ...
    // Single-quoted: double every ' for PowerShell escaping
...
async function notifyWindows(shell: Shell, title: string, body: string): Promise<void> {
  ...
  await shell`powershell.exe -NoProfile -EncodedCommand ${encoded}`.quiet()
}
